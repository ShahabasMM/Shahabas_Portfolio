...
import { ExternalLink, Github, Layers, Download } from 'lucide-react';
...
const ProjectCard = ({ title, description, tags, github, demo, status, fileUrl, fileName, delay }) => {
  ...
  return (
    ...
    viewport={{ once: true }}
    whileHover={{ y: -10 }}
    className="group relative glass rounded-2xl overflow-hidden red-glow-border flex flex-col h-full w-full max-w-[28rem] mx-auto"
  >
    <div className="relative aspect-video overflow-hidden">
      ...
      <div className="absolute top-4 left-4 flex gap-2">
        {tags.map((tag, i) => (
          <span key={i} className="px-3 py-1 bg-black/50 backdrop-blur-md border border-white/10 text-[10px] font-bold uppercase tracking-wider rounded-full text-white">
            {tag}
          </span>
        ))}
      </div>
    ...
    <div className="p-5 flex flex-col flex-grow">
      <h3 className="text-lg font-poppins font-bold mb-2 group-hover:text-primary transition-colors">{title}</h3>
      <p className="text-gray-400 font-inter text-sm leading-relaxed mb-5 flex-grow">
        {description}
      </p>
    ...
    <section id="projects" className="py-24 bg-black/40 relative">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className="section-title text-4xl font-poppins font-bold">Featured <span className="text-primary">Projects</span></h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {projects.map((project, index) => (
            <ProjectCard key={project.id} {...project} delay={0.1 + index * 0.1} />
          ))}
        </div>
